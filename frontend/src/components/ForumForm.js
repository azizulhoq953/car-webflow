...
// import { FaImage, FaRegFileAlt, FaCheck, FaTimes, FaExclamationCircle, FaTrashAlt, FaPen, FaFontAwesome } from 'react-icons/fa';
// import { useDropzone } from 'react-dropzone';

// // FileDropzone Component (same as before)
...
// // Text Modal Component (same as before)
...
//   const [isAdmin, setIsAdmin] = useState(false); // Store the admin status
...
//     try {
//       const response = await axios.get('http://localhost:5000/api/fields'); // Modify this to the correct API endpoint
...
//         setIsAdmin(true); // Set the admin flag if the user is an admin
//       }
//     }
//   }, []);
...
//     try {
//       // Loop through each valid row to submit them individually
...
//         await axios.post(
//           `${process.env.REACT_APP_API_BASE_URL}/forums`, 
//           formData, 
//           {
//             headers: {
...
//       setForumRows([]); // Optionally reset the form rows
//     } catch (error) {
...
//   // const handleDeleteForum = (index) => {
//   //   const newRows = forumRows.filter((_, i) => i !== index);
//   //   setForumRows(newRows);
//   // };
//   const handleDeleteForum = async (index) => {
//     const forumToDelete = forumRows[index];
//     const forumName = forumToDelete.forumName; // Extract the forumName
...
//         await axios.delete(
//           `http://localhost:5000/api/fields/${forumName}`, // API endpoint with forumName
//           {
//             headers: {
...
//   const handleUpdateForum = (index) => {
//     // Implement your update logic here
//     console.log('Update forum at index:', index);
//   };
...
//           {authState.isAdmin && (
//              <button type="button" onClick={() => handleRemoveRow(index)} style={styles.removeButton}>
//              <FaTrashAlt />
//            </button>
//           )}
...
//       <TextModal isOpen={isModalOpen} onClose={handleCloseModal} onSave={handleSaveNote} value={forumRows[currentRowIndex]?.note || ''} />
//     </div>
//   );
// };
...
const ForumForm = () => {
  ...
  const handleSelectSymbol = (symbol) => {
    ...
    console.log('Selected Symbol:', symbol); // Debugging selected symbol
  };
  // Fetch forum names and fields data from the API
  const fetchFields = async () => {
    try {
